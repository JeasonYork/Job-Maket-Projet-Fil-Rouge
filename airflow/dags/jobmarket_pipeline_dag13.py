...
dag = DAG(
    'json_processing_pipeline_13',
    default_args=default_args,
    description='Pipeline to process new JSON files',
    schedule_interval=None,  # No schedule, triggered manually or by FileSensor
...
with dag:
    ...
    new_json_file_sensor = FileSensor(
        ...
        timeout=7 * 24 * 60 * 60,  # Timeout after 1 week (or adjust as needed)
    ...
    # Task to trigger the DAG again
    trigger_next_cycle = TriggerDagRunOperator(
        task_id='trigger_next_cycle',
        trigger_dag_id='json_processing_pipeline_11',
        wait_for_completion=False,
        reset_dag_run=True
    )

    new_json_file_sensor >> list_new_json_files_task >> process_files_task >> bulk_load_task >> trigger_next_cycle
