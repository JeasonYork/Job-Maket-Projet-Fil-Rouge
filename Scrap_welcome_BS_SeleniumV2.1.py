#Version avec webdriver simplifié
#Pas de location, un terme de recherche
...
def get_links(url): # Fait la recherche avec le terme puis recupere les liens vers les offres de la premiere page de résultat
    driver.get(url)
...
def get_info(link): # Recupeère les infos des offres pour les liens trouvé par get_links
    ...
    try : # Tittre de l'offre
        ...
        title = "N/A"
    #try : # Lieu de l'emplois
        #lieu = soup.find('span', class_='sc-1eoldvz-0 bZJPQK').text
    #except :
        #lieu = "N/A"
    try: # Ensemble d'informations sur l'offre : Type de contrat, lieu, salaire, teletravail, niveau d'étude minimum  (à améliorer)
        job_info_tag = soup.find('div', class_='sc-bXCLTC hdepoj') 
        job_info_texts = [job_info.text for job_info in job_info_tag]
    except:
        contrat = lieu = salaire = teletravail = education = "N/A" # Beaucoup d'offres avec structure différente
    try:
