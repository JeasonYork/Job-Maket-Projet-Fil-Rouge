...
async def extract_links(page, job_search_url: str, job_links_selector: str):
    ...
    """
    try:
        logging.info(f"Navigating to {job_search_url}")
        ...
        logging.info(f"Waiting for selector: {job_links_selector}")
        ...
        logging.info(f"Number of elements found: {len(elements)}")
        links = [await element.get_attribute("href") for element in elements]
        logging.info(f"Total extracted links: {len(links)}")
